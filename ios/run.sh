...
EOF
        ...
        "$DOWNLOADS_PATH/fruitstrap/fruitstrap" -b "$IOS_PROJECT_PATH/build/$PROJECT_NAME.app" -t 5 -d -x "$GDB_COMMANDS_FILE" > "$STDOUT" 2>&1 || error "Failed to install to device"
        ...
        "$DOWNLOADS_PATH/fruitstrap/fruitstrap" -b "$IOS_PROJECT_PATH/build/$PROJECT_NAME.app" -t 5 -d -x "$GDB_COMMANDS_FILE" || error "Failed to install to device"
    fi
fi
