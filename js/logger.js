...
define(['jquery', 'underscore', 'stacktrace', 'events'], function($,_,Stacktrace,Events) {
    ...
    Logger.renderStats = function() {
        ...
        if (window.performance && window.performance.memory) {
            if (window.performance.memory.usedJSHeapSize)  memUsed  = Math.round(window.performance.memory.usedJSHeapSize  / 100000) / 10;
            if (window.performance.memory.totalJSHeapSize) memTotal = Math.round(window.performance.memory.totalJSHeapSize / 100000) / 10;
        }
    ...
    Logger.error = function(msg, lvl) {
        var level = lvl || 0;
        Logger.logLines.push({ t:+new Date(), msg:'<span style="color:#f88">' + _.escape(msg) + '</span>', expand:false, level:level });
        // Logger.render();
