...
define(['underscore', 'backbone', 'database'],
function(_, Backbone, Database) {
...
_.extend(Backbone.DBStorage.prototype, {
    ...
    sqlInsert: function(model, callback) {
        var request = "INSERT OR REPLACE INTO " + this.name + " (" + this.columns.join(",") + ") VALUES (" + _(this.columns).map(function(){return'?';}).join(",") + ")";
...
Backbone.DBStorage.prepare = function(model) {
  ...
  if (model.collection) model.collection.whereSQL = model.whereSQL;
};
